...
public class PushMessageNew {
	private static Logger logger = Logger.getLogger(PushMessageNew.class);
	...
	private static BaiduPushClient initPushClient() {
		...
		pair = new PushKeyPair("y14OUOumIyVdbcsZEXbT3Qrn", "EI3jcjLGiwj2PlHtP9YwBN2PUlfK7Syx");
	...
	public static Map<String, Object> androidPushBatchUniMsg(String title, String description, String[] channelIds,
			int deviceType) throws IOException {
		...
		try {
			...
			// Logger.debug(String.format("msgId: %s, sendTime:
			// %d",response.getMsgId(), response.getSendTime()));
	...
	public static Map<String, Object> pushMsgToAll(String title, String description, long sendTime, int expireTime,
			int messageType, int openType, String url, int deviceType, String alert, Object... obj)
		...
		try {
			PushMsgToAllRequest request = new PushMsgToAllRequest().addMsgExpires(new Integer(expireTime)) // 默认
																											// new
																											// Integer(3600)
	 ...
	 * 
	 * @param channelId
	 *            是 唯一对应一台设备 必须为客户端初始化成功之后返回的channelId，默认null。
	 * @param expireTime
	 *            到期时间/死亡时间
	 * @param messageType
	 *            否 消息类型 0：透传消息 1：通知 默认值为0
	 * @param title
	 *            标题
	 * @param description
	 *            描述
	...
	public static Map<String, Object> pushMsgToSingleDevice(String channelId, int expireTime, int messageType,
			String title, String description, int openType, String url, int deviceType, String alert, Object... obj)
			throws PushClientException, PushServerException {
		...
		try {
			PushMsgToSingleDeviceRequest request = new PushMsgToSingleDeviceRequest().addChannelId(channelId)
					.addMsgExpires(expireTime) // message有效时间,单位秒,默认3600*5.
	 ...
	 * @throws PushServerException
	 */
	public static List<?> QueryMsgStatus(String[] msgIds, Integer deviceType)
			throws PushClientException, PushServerException {
		...
		try {
			...
			// + "result:");
			/*
			 * 将通知消息保存到日志中 if( null != response){ List<?> list =
			 * response.getMsgSendInfos(); for (int i = 0; i < list.size(); i++)
			 * { Object object = list.get(i); if(object instanceof MsgSendInfo){
			 * MsgSendInfo msgSendInfo = (MsgSendInfo) object; StringBuilder
			 * strBuilder = new StringBuilder(); strBuilder.append("List[" + i +
			 * "]: {" + "msgId = " + msgSendInfo.getMsgId() + ",status = " +
			 * msgSendInfo.getMsgStatus() + ",sendTime = " +
			 * msgSendInfo.getSendTime() + ",success = " +
			 * msgSendInfo.getSuccessCount()); strBuilder.append("}\n");
