...
<head>
	...
	<title>轮胎站点列表</title>
	<meta name="keywords" content="轮胎站点列表">
    <meta name="description" content="轮胎站点列表">
...
<body>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group mtb0" style="position: relative">
                    <div class="col-sm-3 ">
                        <img id="change" src="${pageContext.request.contextPath}/statics/img/test/1516255090.png" height="150" width="150"/>
                    </div>
                    <div style="cursor: pointer;position: absolute;left:160px;top:20px;">
                        <div class="col-sm-1 ">&nbsp;</div>
                        <div class="col-sm-7 "><a id="downimg" srcimg="" downimg="" onClick="downimg(this);">下载</a></div>
                        <div class="col-sm-1 ">&nbsp;</div>
                        <div class="col-sm-1 ">&nbsp;</div>
	...
	<%@include file="common/header.jsp"%>
        <div class="row wrapper border-bottom gray-bg page-heading">
            ...
            <div class="oli_site">
                <a href="${pageContext.request.contextPath}/gasstation/showlist">加油站点</a><a href="javascript:void(0);">轮胎站点</a>
            </div>
        </div>
                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-title p0">
                            <form method="post" action="showtire" class="form-inline">
                            	<input type="hidden" name="pageIndex" value="1"/>
								<input type="hidden" name="gsType" value="1" />
									<div class="form-group">
										<label for="exampleInputName2">站点名称:</label> <input type="text" name="siteName" class="form-control" value="${siteName}">
										...
										<option value="0"<c:if test="${status==0}">selected="selected"</c:if>>已禁用</option>
										<option value="1"<c:if test="${status==1}">selected="selected"</c:if>>已启用</option>
										</select>
										<button type="submit" class="btn  btn-primary">&nbsp;&nbsp;搜索&nbsp;&nbsp;</button>
									</div>
								<a href="javascript:" onClick="addtire(this);" style="float: right;" class="btn btn-primary">+新增</a>
                            </form>
                        </div>
                        ...
                        <div class="ibox-content pt10">
                            <table class="table table-bordered text-center">
                                ...
                                <tr>
									...
									<td><span>经度:${s.longitude}</span><br>
										<span>纬度:${s.latitude}</span>
									</td>
									<td>${s.tyreBrand}</td>
									<td>${s.contact}</td>
									<td>${s.mobilePhone}</td>
									...
									<td>
                                        <a class="text-primary" href="javascript:" gassid="${s.id}" onClick="updategass(this);">编辑</a>
                                        <a class="text-primary" gassid="${s.id}" index="${pages.currentPageNo}" href="javascript:" onClick="del(this);">删除</a>
                                        <a class="text-primary" href="javascript:" gassid="${s.id}" onClick="selectgass(this);">查看</a>
                                        <a class="text-primary" gassimg="${s.qrCodeImg}" onClick="showMa(this);">二维码</a>
                                        <a class="text-primary" status="${s.status}" gassid="${s.id}" 
                                        index="${pages.currentPageNo}" href="javascript:" onClick="fob(this);">
                                        <c:choose>
											<c:when test="${s.status==0}">启用</c:when>
											<c:when test="${s.status==1}">禁用</c:when>
										</c:choose></a>
										<a class="text-primary" onClick="note(this);" gassid="${s.id}" href="javascript:">记录</a>
                                       <%-- <a class="text-primary" onClick="commit(this);" gassid="${s.id}" href="javascript:">提取</a>--%>
                                        <%-- <a class="text-primary" onClick="note(this);" gassid="${s.id}" href="javascript:">记录</a> --%>
                                    </td>
                                </tr>
                               	</c:forEach>
                                ...
                                <tr class="gray-bg">
                                    ...
                                    <td class="text-right" colspan="3">共${pages.totalCount }条/${pages.totalPageCount}页&nbsp;&nbsp;每页显示${pages.pageSize}条&nbsp;&nbsp;第${pages.currentPageNo}页</td>
                                </tr>
                                </tfoot>
                                </c:if> 
                            </table>
...
<!--弹窗-->
<script src="${pageContext.request.contextPath}/statics/js/demos/tire.js"></script>
