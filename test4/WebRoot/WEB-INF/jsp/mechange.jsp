...
<title>站点审批进度查看</title>
<meta name="keywords" content="站点审批进度查看">
<meta name="description" content="站点审批进度查看">
<link
...
<body>
	...
	<div class="row wrapper border-bottom gray-bg page-heading">
            ...
            <form class="form-inline" method="post" action="showdealer">
            	<input type="hidden" name="pageIndex" value="1" /> 
                <select class="form-control" name="status">
                    <option value="">审批状态</option>
                    <option value="0"<c:if test="${status==0}">selected="selected"</c:if>>待审批</option>
                    <option value="1"<c:if test="${status==1}">selected="selected"</c:if>>已通过审批</option>
                    <option value="2"<c:if test="${status==2}">selected="selected"</c:if>>未通过审批</option>
                </select>
                ...
                <div class="form-group daiti">
                    <div class="num">
                        <div class="oli">
                            <div>待提油量</div>
                            <div><span>${gass.quota}</span>&nbsp;L</div>
                        </div>
                        <div class="tiqu">
                            <div><a href="${pageContext.request.contextPath}/site/dealercommit" style="color:white;">提取</a></div>
                        </div>
                    </div>
                </div>
            </form>
                        ...
                        <div class="ibox-content pt10">
                            <table class="table table-bordered text-center">
                                ...
                                <c:forEach var="s" items="${list}" varStatus="status">
                                	<tr>
                                   	 	<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${s.applicationTime}"/></td>
                                  	  	<td>${s.amountDrawn}吨/${s.amountDrawn*ptt}L</td>
                                   		 <td>${s.surplusBalance}L</td>
                                   		 <td>${gass.code}</td>
                                   		 <td>${s.telePhone}</td>
                                   		 <td>
                                   		 <c:choose>
											<c:when test="${s.status==0}">待审批</c:when>
											<c:when test="${s.status==1}">已通过审批</c:when>
											<c:when test="${s.status==2}">未通过审批</c:when>
										</c:choose>
										</td>
                                   		 <td>${s.emplId}</td>
                                   		 <td>${s.serialNum}</td>
                                   		 <td><fmt:formatDate pattern="yyyy-MM-dd" value="${s.approveData}"/></td>
                                	</tr>
                                </c:forEach>
                                </tbody>
                                <tfoot>
