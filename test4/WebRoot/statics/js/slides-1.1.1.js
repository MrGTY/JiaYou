	...
	a.fn.olvSlides = function(b) {
		...
		return this.each(function() {
			...
			if (b.thumb) {
				...
				if (b.thumbPage) {
					g.append('<a href="#Prev" hidefocus="true" class="' + b.thumbPrev + '"></a><a href="#Next" hidefocus="true" class="' + b.thumbNext + '"></a>')
				}
				if (b.thumbDirection === "Y") {
					D = C.outerHeight();
					p = C.children(":eq(0)").outerHeight();
					f = Math.ceil(D / p);
					s = -p * H + D;
					F = C.children(":eq(0)").position().top;
					C.children(":eq(0)").css({
				...
				C.children(":eq(0)").children().each(function(L) {
					...
					var K = '<li><div class="imgWrap"><a href="' + J + '" title="' + I + '"><img src="' + N + '" alt="' + I + '" /></a></div></li>';
					a(this).data("data", L);
					q.append(K)
				});
